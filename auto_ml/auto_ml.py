...
class AutoML(object):
    ...
    def cross_val_train(self, data, k=4):
        """If training a neural net, returns a list of tensorflow.python.keras.callbacks.History objects to plot the 
        model loss and model accuracy across each fold. If training a scikit model, it will return None and simply print 
        the model.score to the screen.
        
        Cross validation still hold out the test set for the evaluate() function later. The predictions for that test 
        set will be generated by the last trained model in cross validation. Therefore if we train over 4 folds with a
        dataset with 200k records, we have:
            all records                        = 200k
            test set excluded                  = 160k
            train, val at each fold            = 120k, 40k"""
        ...
        for train, val in KFold(n_splits=k).split(X_train, y_train):
            ...
            if self.active_model == 'neural_net':
                ...
                self.train_scikit()
                validation_score = self.model.score(self.X_val, self.y_val)
    ...
    def train(self, data, save=True, cross_val=True):
        ...
        if cross_val: 
            ...
            print('\nTraining on {}\nValidating on {}\nTesting on {}...\n'.format(self.X_train.shape[0], self.X_val.shape[0], self.X_test.shape[0]))
...
class AutoMLRegressor(AutoML):
    ...
    def predict(self, X_test):
        print('Predicting {} test samples...'.format(X_test.shape[0]))
